			...
			<div
				...
				{cart.length !== 0 ? (
					<div className='mt-16 md:mt-10'>
						{cart.map(item => (
							<div className='bg-white flex justify-between mb-5 items-center px-4 py-2 rounded-xl drop-shadow-md' key={item.product.id}>
								<div className='flex flex-col justify-center items-center'>
									<img className='w-40' src={item.product.image_url} alt="" />
									<h2 className='font-story text-xl'>{item.product.name}</h2>
								</div>
								<p className='font-semibold text-lg'>{item.product.price}$</p>
								<p className='font-semibold'>{item.total}</p>
								<p className='font-semibold text-main/90'>{item.total * item.product.price}$</p>


								<div className='flex flex-col justify-between items-center gap-2'>
									<IoMdAddCircleOutline className='w-6 h-6 hover:text-green-600 hover:scale-110 transition-fast cursor-pointer select-none' onClick={() => plusCart({ id: item.product.id })} />

									<IoMdRemoveCircleOutline className='w-6 h-6 hover:text-orange-400 hover:scale-110 transition-fast cursor-pointer select-none' onClick={() => minusCart({ id: item.product.id })} />

									<FaRegTrashAlt className='w-5 h-5 hover:text-main hover:scale-110 transition-fast cursor-pointer' onClick={() => removeFromCart({ id: item.product.id })} />
