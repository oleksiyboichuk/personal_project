...
export const ProductBlock: FC<ProductBlockProps> = ({ id, name, price, image, region, product, cart, discount }) => {
	...
	const handleClickFavorites = (event: React.MouseEvent<SVGElement, MouseEvent>) => {
		event.preventDefault()
		toggleFavorites(product)
	}

	const handleClickAddToCart = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
		event.preventDefault()
		if (!isExistsInCart) {
			addToCart(product)
		}
			...
			<Link to={`${pathname}/${id}`}>
				<section className='bg-white max-w-[300px] rounded-3xl px-5 py-4 m-3  drop-shadow-xl hover:scale-105 transition-slow cursor-pointer z-10' key={id}>
					...
					<div className='flex justify-between items-center'>
						...
						<button className={`text-white text-xl px-2  py-1 rounded-lg my-2 font-semibold hover:scale-105 active:bg-white ${!isExistsInCart ? 'bg-green-500 hover:bg-green-400 ' : 'bg-main hover:bg-main/80'} transition-slow`} onClick={handleClickAddToCart}>
